...
class AboutLists(Koan):
    ...
    def test_slicing_lists(
        self,
    ):  # https://www.w3schools.com/python/python_lists_access.asp *slice; meaning taking from a list and adding to a new list*
        noms = ["peanut", "butter", "and", "jelly"]
    ...
    def test_slicing_to_the_edge(self):
        ...
        self.assertEqual(
            ["and", "jelly"], noms[2:]
        )  # slices everything after 2nd index
        self.assertEqual(
            ["peanut", "butter"], noms[:2]
        )  # slices everything before 2nd index

    def test_lists_and_ranges(
        self,
    ):  # https://www.w3schools.com/python/ref_func_range.asp *talks about range* ----> range(start, stop, step)
        self.assertEqual(range, type(range(5)))
        self.assertNotEqual([1, 2, 3, 4, 5], range(1, 6))
        self.assertEqual([0, 1, 2, 3, 4], list(range(5)))
        self.assertEqual(
            [5, 6, 7, 8], list(range(5, 9))
        )  # returns the numbers including 5 and up to 9 but not including 9
    ...
    def test_insertions(self):
        ...
        knight.insert(2, "not")  # tells what index to pass in the value to the list
